...
export async function assembleChainMetadata(
  ...
  if (!result.success) {
    console.warn('Invalid chain metadata', result.error);
  ...
  if (config.registryUrl) {
    console.debug('Using custom registry metadata from:', config.registryUrl);
    registryChainMetadata = await registry.getMetadata();
  } else {
    console.debug('Using default published registry');
